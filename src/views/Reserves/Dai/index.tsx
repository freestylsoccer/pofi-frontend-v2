...
import { getDaiAddress } from 'utils/addressHelpers'
import { getBalanceNumber } from 'utils/formatBalance'
import { PROTOCOL_DATA_PROVIDER_ADDRESS, PROJECT_DAI } from 'config/constants'
...
const TrueUsd: React.FC = () => {
  ...
  const { liquidity, fetchStatus } = useReserveLiquidity(PROTOCOL_DATA_PROVIDER_ADDRESS, PROJECT_DAI)
  const { t } = useTranslation()

  const decimals = 18;
  const projectTotalDeposited = getBalanceNumber(liquidity, decimals);
  const { aTokenBalance, balanceFetchStatus } = useReserveUserDeposited(PROTOCOL_DATA_PROVIDER_ADDRESS, PROJECT_DAI, account)
            ...
            <div className="container content-end">
              ...
              <div>
                <div className="pb-3">
                  ...
                  <Title>${`${projectTotalDeposited.toLocaleString('en-US')} DAI`}</Title>
                  <NormalText>
                    <NormalTextSpanIII>You have deposited: </NormalTextSpanIII>
                    <NormalTextSpanIII>${`${getBalanceNumber(aTokenBalance, decimals).toLocaleString('en-US')} DAI`}</NormalTextSpanIII>
                  </NormalText>
                </div>
              </div>
            </div>
            ...
            <div className="iXjtZw" >
              { getBalanceNumber(liquidity, 6) < maxAmount ? (
                <Button
                  type="button"
                  as={Link}
                  to={`/add/${getDaiAddress()}/${PROJECT_DAI}`}
                >
                {t('Invest')}
                </Button>
              ) : getBalanceNumber(liquidity, 6) >= maxAmount && (
